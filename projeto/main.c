...
void startPath(int cidade_atual, int cidade_objetivo, int qtd_cidades, int *menor_distancia, int menor_caminho[], LADJ *listas){
    ...
    for(int i = 0; i < arestas; i++){ // insere as conexões entre as cidades e a distancia entre elas.
...
int main(void){ 
    ...
    LADJ *listas = ListaAdj_criar(qtd_cidades);
    ...
    startPath(cidade_origem, cidade_origem, qtd_cidades, &menor_distancia, menor_caminho, listas);
